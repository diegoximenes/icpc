...
Version: VERTEX AND EDGE disjoint
...
int main()
{
	int u, v, w, m, naux, s, t;

	while(scanf("%d %d", &naux, &m) == 2)
	{
		n = 2*naux; s = 0; t = naux - 1;
		for(int i=0; i<n; ++i) g[i].clear();
		
		for(int i=0; i<naux; ++i)
			g[in(i)].pb(edge(out(i), 0, 0));

		while(m--)
		{
			scanf("%d %d %d", &u, &v, &w);
			--u; --v;
			g[out(u)].pb(edge(in(v), w, 0));
		}
	
		printf("%d\n", suurballe(out(s), in(t)));
	}
