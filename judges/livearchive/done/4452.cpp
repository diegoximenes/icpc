...
Description: (a v b) ^ (not(a) v c) ^ ... ^ (not(c) v d). n is the number of boolean variables
...
To find a configuration: force a to be true (add edge (not(a), a)) and verify if there is a valid configuration. 
Else verify if there is valid configuration with a = false (add egde(a, not(a))). Do this with all variables.
...
//condition variable i to be true. If this condition satisfy return true, else return false
inline bool conditionVariable(int i)
{
	g[notId(i)].pb(i);
	initTarjan();
	bool can = verify();
...
int main()
{
	for(int tc=1; scanf("%d %d", &m, &b) && b; ++tc)
	{
		...
		while(b--)
		{
			...
			else
			{
				for(int i=0; i<k; ++i)
					for(int j=i+1; j<k; ++j)
					{
						g[notId(vote[i])].pb(vote[j]);
						g[notId(vote[j])].pb(vote[i]);
					}
			}
		}
		...
		if(sat)
		{
			for(int i=0; i<n; i+=2)
			{
				bool a = conditionVariable(i), b = conditionVariable(notId(i));
				if(a && b) putchar('?');
				else if(b) putchar('y');
				else putchar('n');
			}
