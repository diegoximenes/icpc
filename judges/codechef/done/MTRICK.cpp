...
//MAXD is the number of digits

const int MAXD = 40, DIG = 9, BASE = 1000000000;
...
bignum l[MAXN], a, b, c, cum, mul;
char str[MAXD], s[MAXN];
	...
	while(t--)
	{
		...
		for(int i=0; i<n; ++i)
		{
			if(s[i] == 'A')
			{
				...
				if(pleft)
				{
					lout.pb((l[pl]*mul + cum)%c);
					pl = pl + 1; 
				}
				else
				{
					lout.pb((l[pr]*mul + cum)%c);
					pr = pr - 1;
				}
			}
			else if(s[i] == 'M')
		...
		printf("%s", lout[0].str());
		for(int i=1; i<n; ++i) printf(" %s", lout[i].str());
		puts("");
	}			
    return 0;
}
