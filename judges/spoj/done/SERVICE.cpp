...
int v[MAXQ], cnt[MAXN], cost[MAXN][MAXN], dp[MAXQ][MAXN][MAXN], n, q;
		...
		for(int i=q-1; i>=0; --i)
			for(int l1=0; l1<n; ++l1)
				for(int l2=0; l2<n; ++l2)
				{
					...
					int sol1;
					if(i == 0) sol1 = cost[0][v[i]] + dp[i+1][l1][l2];
					else sol1 = cost[v[i-1]][v[i]] + dp[i+1][l1][l2];
					int sol2 = cost[l1][v[i]] + dp[i+1][v[i-1]][l2];
					int sol3 = cost[l2][v[i]] + dp[i+1][l1][v[i-1]];
						
					dp[i][l1][l2] = min(sol1, min(sol2, sol3));
				}
