...
const int MAXD = 3000, DIG = 9, BASE = 1000000000;
...
int main ()
{
	...
	{
		count[0][0] = count[0][1] = count[1][0] = count[1][1] = 1;
		for(int i=2; i<k; ++i)
			count[i][0] = count[i][1] = count[i-1][0] + count[i-1][0];

		for(int i=max(k, 2); i<=n; ++i)
		{
			count[i][0] = count[i-1][0] + count[i-1][1];
			int ind = i-k-1;
			if(ind >= 0)
				count[i][1] = count[i][0] - count[ind][0];
			else
				count[i][1] = count[i][0];
		}
		printf("%s\n", (count[n][0] + count[n][1]).str());
	}
